...
if __name__ == '__main__':
    ...
    if e_model == 'depolarized':
        ...
        h = args.h
        E = Errormodel()
        errors = E.ising_error(n_s=trials, beta=beta, h=h)
    ...
    van = MADE(g.size(0), depth, width, activator='tanh').to(device).to(dtype)
    ...
    for l in range(epoch):
        if e_model == 'depolarized':
            ...
            ers = E.ising_error(n_s=batch, beta=beta, h=h)
       
        configs = E.configs(sta=Code.g_stabilizer, log=Code.logical_opt, pe=e1, opts=ers, seq=seq)
        s = configs*2 -1
        logp = van.log_prob(s)
        ...
        loss.backward()
        optimizer.step()
        if optimizer.state_dict()['param_groups'][0]['lr'] > 0.004 :
            None#scheduler.step()
    ...
    if save == True:
        if e_model == 'depolarized':
            path = abspath(dirname(__file__))+'/net/fkl_made_'+c_type+'_d{}_k{}_seed{}_er{}_{}.pt'.format(d, k, seed, er, seq)
            if exists(path):
                None
            else: 
                torch.save(van.deep_net, path)
        elif e_model == 'ising':
            path = abspath(dirname(__file__))+'/net/fkl_made_'+c_type+'_d{}_k{}_seed{}_ising{}_h{}_{}.pt'.format(d, k, seed, beta, h, seq)
            if exists(path):
                None
            else: 
                torch.save(van.deep_net, path)  
