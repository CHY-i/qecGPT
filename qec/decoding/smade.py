...
if __name__ == '__main__':
    ...
    epoch, batch, lr = 1000000, args.batch, args.lr
    ...
    if save == True:
        if e_model == 'depolarized':
            path = abspath(dirname(__file__))+'/net/ds/fkl_made_'+c_type+'_d{}_k{}_seed{}_er{}_{}1.pt'.format(d, k, seed, er, seq)
            if exists(path):
                None
            else:          
                torch.save(van, path)
        elif e_model == 'ising':
            path = abspath(dirname(__file__))+'/net/ds/fkl_made_'+c_type+'_d{}_k{}_seed{}_ising{}_h{}_{}.pt'.format(d, k, seed, beta, h, seq)
            if exists(path):
                None
            else:  
                torch.save(van, path)  
